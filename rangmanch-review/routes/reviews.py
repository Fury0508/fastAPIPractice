...
def list_reviews(play_name:str |None = Query(None, description = "Filter by play name"), skip:int  = Query(0,ge=0,description="Number of reviewt to skip"),
limit:int = Query(10,ge=1,le=50, description = "Max reviews to return"),session: Session= Depends(get_session)):
    query = select(Review)
    if play_name:
...
def get_average_rating(play_name:str,session: Session= Depends(get_session)):
    ...
    avg_rating, total_review = result
    ...
    return {
        "play_name": play_name,
        "average_ratiing": round(avg_rating,2),
